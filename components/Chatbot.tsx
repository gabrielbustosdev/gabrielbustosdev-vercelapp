  ...
  initialMessages: [
    {
      ...
      content:
        "¡Hola! 👋 Soy el asistente virtual de Gabriel Bustos. ¿En qué puedo ayudarte hoy sobre desarrollo web, IA o automatización?",
    },
  ],
};
...
export default function ChatBot() {
  ...
  useEffect(() => {
    const lastUser = [...messages]
      .reverse()
      .find((msg) => msg.role === "user" && msg.id !== "welcome");
    if (
      lastUser &&
  ...
  useEffect(() => {
    if (status === "ready") {
      const lastAssistant = [...messages]
        .reverse()
        .find((msg) => msg.role === "assistant" && msg.id !== "welcome");
      if (
        lastAssistant &&
  ...
  const showScheduleButton =
    lastAssistant && shouldShowScheduleButton(lastAssistant.content);
  ...
  const handleOpenModal = async () => {
    ...
    try {
      const res = await fetch("/api/summary", {
        ...
        body: JSON.stringify({
          messages: persistedMessages.map(({ role, content }) => ({
            role,
            content,
          })),
        }),
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((message) => (
            ...
            >
              ...
              >
                <div
                  className={`w-8 h-8 min-w-[2rem] min-h-[2rem] rounded-full flex items-center justify-center flex-shrink-0 aspect-square ${message.role === "user" ? "bg-blue-500" : "bg-gradient-to-r from-slate-600 to-zinc-600"}`}
                >
                  {message.role === "user" ? (
                    <svg
                ...
                <div
                  className={`rounded-2xl px-4 py-3 ${message.role === "user" ? "bg-blue-500 text-white" : "bg-white/10 text-gray-100 border border-white/10"}`}
                >
